...
const (
	...
	MsgAutoContinueStarted             MsgKey = "auto_continue_started"
	MsgAutoContinueLimit               MsgKey = "auto_continue_limit"
	MsgAgentSessionResetAfterBadOutput MsgKey = "agent_session_reset_after_bad_output"
...
var messages = map[MsgKey]map[Language]string{
	...
	MsgAutoContinueStarted: {
		LangEnglish:            "⚠️ The response hit the output limit. Continuing in the same session…",
		LangChinese:            "⚠️ 回复达到输出上限，正在同一会话自动续写…",
		LangTraditionalChinese: "⚠️ 回覆達到輸出上限，正在同一會話自動續寫…",
		LangJapanese:           "⚠️ 応答が出力上限に達しました。同じセッションで自動的に続きを生成します…",
		LangSpanish:            "⚠️ La respuesta alcanzó el límite de salida. Continuando en la misma sesión…",
	},
	MsgAutoContinueLimit: {
		LangEnglish:            "⚠️ I auto-continued 5 times but still hit the output limit. Send “continue” to keep going manually, or split the task into smaller parts.",
		LangChinese:            "⚠️ 已自动续写 5 次，但仍达到输出上限。你可以发送“继续”手动续写，或把任务拆小。",
		LangTraditionalChinese: "⚠️ 已自動續寫 5 次，但仍達到輸出上限。你可以傳送「繼續」手動續寫，或把任務拆小。",
		LangJapanese:           "⚠️ 5 回自動継続しましたが、まだ出力上限に達しています。手動で続けるには「continue」を送るか、タスクを小さく分けてください。",
		LangSpanish:            "⚠️ Continué automáticamente 5 veces, pero aún se alcanzó el límite de salida. Envía “continuar” manualmente o divide la tarea en partes más pequeñas.",
	},
	MsgAgentSessionResetAfterBadOutput: {
		LangEnglish:            "⚠️ The previous Claude Code stream produced an invalid/truncated tool call or zero-token empty result. I reset this agent session so the next message starts from a clean session; use project files/progress notes to continue.",
		LangChinese:            "⚠️ 上一个 Claude Code 会话出现工具调用截断/结构损坏或 0-token 空结果。我已重置该 Agent 会话；下一条消息会从干净会话启动，并应根据项目文件/进度记录继续。",
		LangTraditionalChinese: "⚠️ 上一個 Claude Code 會話出現工具呼叫截斷/結構損壞或 0-token 空結果。我已重置該 Agent 會話；下一則訊息會從乾淨會話啟動，並應根據專案檔案/進度記錄繼續。",
		LangJapanese:           "⚠️ 直前の Claude Code ストリームでツール呼び出しの切断/構造破損、または 0-token の空結果が発生しました。このエージェントセッションをリセットしました。次のメッセージはクリーンなセッションから開始し、プロジェクトファイル/進捗メモを元に続行します。",
		LangSpanish:            "⚠️ El flujo anterior de Claude Code produjo una llamada de herramienta inválida/truncada o un resultado vacío de 0 tokens. Restablecí esta sesión del agente; el próximo mensaje empezará desde una sesión limpia y debe continuar usando los archivos/notas de progreso del proyecto.",
	},
	MsgPermissionPrompt: {
