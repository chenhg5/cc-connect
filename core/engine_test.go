...
func TestEngineReceiptCommandsUseReceiptEnvelopeAndLocalizedReplies(t *testing.T) {
	...
	if !p.deleted || p.reconstructed != "test:secretary" {
		t.Fatalf("receipt did not delete source and route target: deleted=%v target=%q", p.deleted, p.reconstructed)
	}
...
func TestEngine_DisabledCommands_Inline(t *testing.T) {
	...
	// Test web
	p.sent = nil
	blocked = e.handleCommand(p, msg, "/web page")
	if !blocked {
		t.Error("expected web to be blocked")
	}
	sent = p.getSent()
	if len(sent) != 1 || !strings.Contains(sent[0], "disabled") {
		t.Errorf("expected disabled message for web, got: %v", sent)
	}

	// Test ps
	p.sent = nil
	blocked = e.handleCommand(p, msg, "/ps list")
	if !blocked {
		t.Error("expected ps to be blocked")
	}
	sent = p.getSent()
	if len(sent) != 1 || !strings.Contains(sent[0], "disabled") {
