...
Every source file is hashed during preflight and the complete result is built and verified in sibling staging directories. Immediately before activation, migration rebuilds the full source inventory; any added or deleted file, changed content, changed project discovery, or changed project-local access metadata fails closed without activating an incomplete target. Stable destinations are then activated with atomic renames, and earlier activations are rolled back if a later one fails. Project-local `.cc-connect-next` trees preserve the source `.cc-connect` directory/file modes and ownership for `run_as_user` compatibility. Runtime-only logs, sockets, locks, restart notifications, and daemon metadata are excluded; source symlinks are skipped. Existing targets are refused by default. With an explicit `--force`, the previous target is preserved as a timestamped `*.pre-migration-*` backup before activation. The result includes `migration-manifest.json` with every source, destination, size, and SHA-256. Use `--skip-project-data` only when project-local images and attachments are intentionally not wanted.
