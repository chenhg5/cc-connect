...
func (p *Platform) buildSession() (*discordgo.Session, error) {
	...
	session.AddHandler(func(s *discordgo.Session, m *discordgo.MessageCreate) {
		...
		if chatName, _ := p.ResolveChannelName(m.ChannelID); chatName != "" {
...
func (p *Platform) handleInteraction(s *discordgo.Session, i *discordgo.InteractionCreate) {
	...
	if resolvedName, _ := p.ResolveChannelName(channelID); resolvedName != "" {
		msg.ChatName = resolvedName
	}
	p.dispatchMessage(msg)
}
...
func (p *Platform) resolveChannelName(channelID string) string {
	name, _ := p.ResolveChannelName(channelID)
	return name
}
