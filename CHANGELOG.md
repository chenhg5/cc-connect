...
### Added
- **Grok Build agent** (`type = "grok"`): first-class, non-ACP adapter for xAI's native Grok Build CLI. It runs headless turns with `--output-format streaming-messages-json`, resumes the exact saved session with `--resume`, inherits Grok's local default model unless `model` is configured, and supports native permission modes. Omitted `mode` uses the safe native `default`; unrestricted `yolo` requires explicit opt-in and is the only mode that adds `--always-approve`. Custom homes (`GROK_HOME`) and model endpoints (`GROK_MODELS_BASE_URL`) are supported. See `config.example.toml` and install Grok Build from https://x.ai/cli.
