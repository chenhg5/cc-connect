...
- **dsh agent**: new agent adapter for the DeepSeek Harness (dsh). Bridges to a running `dsh web` server (default `http://127.0.0.1:3080`) via its typed RPC API (`POST /api/session.prompt`, `GET /api/events.mux` WebSocket). Sessions are created per-project cwd and resume automatically; streams text/reasoning deltas and tool calls, round-trips approval requests and AskUserQuestion through `/api/respond`, supports `/stop` (session.cancel), per-turn `timeout_mins`, model override via `session.selectModel`, `/list` + session-id validation, and `cc-connect doctor` reachability checks. CC_PROJECT / CC_SESSION_KEY are injected into the first prompt of fresh sessions so the in-session `cc-connect` CLI works. Requires `dsh web` running on the same machine.
