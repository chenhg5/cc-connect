...
- **feishu**: fix silent message loss in multi-project setups where multiple projects use **different** Feishu `app_id` in one process (#1562, related #1143 #733). The legacy `sharedWSGroup` package-level state was always keyed by `app_id+"|"+domain`, but `Platform.Start` and the `Platform.Stop` warning assumed a single shared connection per process; when two projects pointed at two different bots the second project's events were swallowed and `Stop` left the first stranded without a warning. The default now opens **one independent `larkws.Client` per project**. Operators who intentionally fan-out across multiple projects sharing one Feishu `app_id` (the original fan-out use case) opt back in via `share_ws = true` in the platform config. Fan-out errors are now logged with `project` / `app_id` / `platform` keys so a silently misbehaving sibling is grep-able instead of invisible. Covered by six new unit tests in `platform/feishu/feishu_start_test.go` (independent-mode lifecycle, shared-mode regression, default-independent sanity, primary-shutdown warning, fan-out error visibility, no goroutine leak on Stop).
