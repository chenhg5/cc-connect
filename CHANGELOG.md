...
=======
- **core: keep live agent on disk-vs-live mismatch** (#1470). `getOrCreateInteractiveStateWith` previously recycled the live agent process whenever its reported session id disagreed with `Session.AgentSessionID` on disk. The live process is the user's in-flight conversation — killing it discards context as collateral damage. The check is now strictly bounded to the cold-start race window (live alive but no id yet, disk has id to resume from); all other cases reuse the live state. Recycle-on-mismatch fired for every prompt-typed custom command (`[[commands]]` with `prompt = ...`), skill invocation, cron / timer reusing session, and heartbeat when disk was stale, producing a silent ping-pong between two session ids and a "session too small" failure for commands that needed context. (`/new` and `/switch` are unaffected — `cleanupInteractiveState` removes the state from the map before the next message arrives.) Two regression tests added: `TestSessionMismatch_KeepsLiveWhenDiskIsStale`, `TestSessionMismatch_KeepsLiveWhenDiskCleared`, plus the cold-start recycle case `TestSessionMismatch_RecyclesColdStartWhenLiveHasNoId`, and the user-journey test `TestCUJ_G7_CustomCommandKeepsActiveSession` in `core/cuj_test.go`.
- **core: route custom commands and skills to user's chat session in multi-workspace mode** (#1470). `executeCustomCommand` and `executeSkill` were looking up the Session via `sessions.GetOrCreateActive(interactiveKey)`, where `interactiveKey` is `workspace + ":" + sessionKey`. But the SessionManager keys active sessions by the raw `sessionKey` (matching the user-message path at `engine.go:2923` and cron/timer paths). In workspaces with overrides or legacy pollution where both keys coexisted in `active_session`, the command locked a different session from the one the user was chatting in — so e.g. a custom compact-style command would compress a tiny "default" session instead of the real conversation. Switched to `sessions.GetOrCreateActive(msg.SessionKey)` for the lookup; the workspace-prefixed `interactiveKey` is still correct for the engine's `interactiveStates` map. Regression tests: `TestExecuteCustomCommand_LandsInUserChatSession`, `TestExecuteSkill_LandsInUserChatSession` in `core/multi_workspace_test.go`.
>>>>>>> 4405b8a3 (test(core): add CUJ test for /custom-command keeping live session (issue #1470))
