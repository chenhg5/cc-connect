...
- **weixin**: realign send-volume quota defaults with the documented short-window burst guard and make `burst_limit = 0` truly disable the quota (#1716). The default `burst_window_secs` is now `1800` (30 minutes) instead of `86400` (24h), so the quota paces separate messages within a short window instead of capping the bot at 4 messages per day. `burst_limit = 0` now disables the quota as documented rather than silently falling back to the default limit. This is a behavior change for configs relying on the previous 24h default — set `burst_window_secs` explicitly to keep the old window.
