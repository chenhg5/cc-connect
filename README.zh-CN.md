...
出于安全考虑，如果实际生效的 `data_dir` 包含官方配置根目录（例如 `data_dir = "~"`），迁移会直接拒绝执行。这样的目录并非 CC Connect 专用目录，递归复制可能把 SSH 密钥、浏览器资料或其他无关文件一起带走。请先把官方安装指向专用数据目录并确认状态，再重新迁移；遇到这种情况，命令不会静默生成不完整目标。
