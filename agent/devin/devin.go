...
// first-class cc-connect-next agent.
//
...
//  3. Reports Name() = "devin" so cc-connect-next's session store keys,
...
// reads the credentials stored on disk, so cc-connect-next never needs to
