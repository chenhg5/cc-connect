...
每个会话会常驻一个 agent 子进程（以 Claude Code 为例约 300 MB，还带一组 MCP server 子进程）。小内存机器可以让 cc-connect 自动关闭空闲的 agent 子进程、下次需要时再拉起：

```toml
[[projects]]
name = "demo"
idle_exit_mins = 30   # 0 或不设置表示禁用（默认）
```

会话空闲 30 分钟后，agent 子进程（连同它的 MCP 子进程）会被关闭、内存立即释放。对话不会丢失：下一条消息会用已保存的 agent session ID 透明地恢复会话，代价是几秒钟的冷启动。正在执行任务、等待权限确认、或有排队消息的会话不会被关闭。

它与 `reset_on_idle_mins` 的区别：空闲退出释放的是*进程*、保留对话；空闲重置则是切换到*新对话*。两者可以同时启用。
