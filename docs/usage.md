...
Each session keeps a resident agent subprocess (for Claude Code, ~300 MB plus its MCP server children). On small-RAM machines you can let cc-connect close idle agent subprocesses and resume them on demand:

```toml
[[projects]]
name = "demo"
idle_exit_mins = 30   # 0 or unset disables (default)
```

After 30 minutes with no activity, the agent subprocess (and its MCP children) is closed and its memory released. The conversation is not lost: the next message transparently resumes it via the saved agent session ID, at the cost of a few seconds of cold start. Sessions with a running turn, a pending permission request, or queued messages are never closed.

This differs from `reset_on_idle_mins`: idle exit frees the *process* but keeps the conversation; reset-on-idle rotates to a *new conversation*. They can be combined.
