...
If the terminal full-card update itself fails, cc-connect-next creates one completed replacement card with a deletable message handle, then removes the stale lifecycle card. It never turns an already-created lifecycle card into untracked multi-part fallback replies: a recall racing the replacement cleans up both card handles and prevents history commit. Only when the initial lifecycle card could not be created at all does the runtime retain the legacy readable normal-reply degradation path. If a turn fails earlier or its Agent event channel closes unexpectedly, only the last body confirmed by a successful Feishu create/update call may be retained on the failure card or in assistant history. Text held back by disabled previews, throttling, or a failed update is never treated as visible. If the failure-state update also fails, the fallback reply contains only that confirmed safe partial plus localized static failure copy; raw provider/process errors are never substituted into chat-visible text.
